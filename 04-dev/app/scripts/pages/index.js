...
import tplCircuit from '../templates/circuit.hbs'
...
export function circuits(ctx) {
  const u = url.parse(ctx.path, true)
  const currentPage = Math.max(1, u.query.page ? u.query.page : 1)

  fetch(config.api.url + `/circuits.json?limit=${limit}&offset=${limit * (currentPage - 1)}`)
    .then(response => {
...
export function circuit(ctx) {
  fetch(config.api.url + `/circuits/${ctx.params.circuit}.json`)
    .then(response => {
      if (response.status >= 400) {
        return page('error')
      }
